...
import {
  bPieceExistsOnTargetTopLeft,
  bOtherExistsOnTopLeft,
 ...
 * _ _ _ _ w
 */
describe('bOtherExistsOnTopLeft', () => {
  it('returns an object { targetPiece: null, pieceExists: false } \
    if no piece exists on the top left diagonal from a given source', () => {
    // Arrange
    const board = getBoard()
    const coords = { nRow: 1, nCol: 5, nDestRow: 5, nDestCol: 1 }
    board[0][4] = {
      nRow: 1, nCol: 5, bHasWhiteChip: true, bHasBlackChip: false, bHasWhiteKing: true, bHasBlackKing: false
    }
    ...
    // Act
    const result = bOtherExistsOnTopLeft(board, coords, bSourceHasWhite, bSourceHasBlack)
  ...
  describe('White Source Piece', () => {
    it('returns an object { targetPiece: null, pieceExists: false } \
      if the first piece seen on the top left diagonal from a given source is white.', () => {
      // Arrange
      const board = getBoard()
      const coords = { nRow: 1, nCol: 5, nDestRow: 5, nDestCol: 1 }
      board[0][4] = {
  ...
  describe('Black Source Piece', () => {
    it('returns an object { targetPiece: null, pieceExists: false } \
      if the first piece seen on the top left diagonal from a given source is black.', () => {
      // Arrange
      const board = getBoard()
      const coords = { nRow: 1, nCol: 5, nDestRow: 5, nDestCol: 1 }
      board[0][4] = {
      ...
      // Act
      const result = bOtherExistsOnTopLeft(board, coords, bSourceHasBlack, bSourceHasWhite)
...
describe('bOtherExistsAfterSource', () => {
  ...
  describe('Black Source', () => {
    it('returns an object { targetPiece: null, pieceExists: false } \
      ...
      const coords = { nRow: 1, nCol: 5, nDestRow: 5, nDestCol: 1 }
      board[0][4] = {
        nRow: 1, nCol: 5, bHasWhiteChip: false, bHasBlackChip: true, bHasWhiteKing: true, bHasBlackKing: true
      }
...
describe('bPieceExistsOnTopLeft', () => {
  it('returns true if a piece exists between the target piece and the destination square', () => {
    ...
    board[3][1] = {
      nRow: 4, nCol: 2, bHasWhiteChip: false, bHasBlackChip: true, bHasWhiteKing: false, bHasBlackKing: false
    }

    const expected = true
    
    // Act
    const result = bPieceExistsOnTargetTopLeft(board, coords, targetPiece)
...
describe('bPieceExistsBetweenTargetAndDest', () => {
  ...
  it('returns true if a piece exists at the destination square', () => {
    ...
    board[4][0] = {
      nRow: 5, nCol: 1, bHasWhiteChip: true, bHasBlackChip: false, bHasWhiteKing: false, bHasBlackKing: false
    }
...
describe('bIsValidCapture', () => {
  describe('Friendly piece exists between source and target', () => {
    it('returns false if the first piece encountered from the source is a friendly piece', () => {
      ...
      board[1][3] = {
        nRow: 2, nCol: 4, bHasWhiteChip: true, bHasBlackChip: false, bHasWhiteKing: false, bHasBlackKing: false
      }
  ...
  describe('Destination already occupied', () => {
    it('returns false if there exists a piece at the destination square', () => {
      ...
      board[4][0] = {
        nRow: 5, nCol: 1, bHasWhiteChip: false, bHasBlackChip: true, bHasWhiteKing: false, bHasBlackKing: false
      }
